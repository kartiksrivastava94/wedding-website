...
// Non-cocktail events — shown to every guest. Keep in sync with the Schedule page.
const EVENTS = [
...
export default function RsvpForm({ cocktail, dec1Stay, action }) {
  ...
  const [flights, setFlights] = useState("Not yet");
  ...
  const accepting = attending === "Joyfully accept";
    ...
    <form action={formAction} className="form">
      ...
      {accepting && (
        <>
          ...
          <p className="checks">
            <label>
              <input type="checkbox" name="whatsapp" value="yes" defaultChecked />{" "}
              You can add me to the WhatsApp group(s) for planning updates
              (we&rsquo;ll use the number above).
            </label>
          </p>
          <p>
            <label>
          ...
          {cocktail && !dec1Stay && (
            <p>
              <label>
                Arriving for the Dec 1 pool party &amp; cocktails (need the Dec 1
                night)?
                <br />
                <select
                  className="select"
