...
// ---- Admin (RSVP dashboard) auth ----
// A single shared admin password (ADMIN_PASSWORD), separate from guest codes.
...
export function checkAdminPassword(pw) {
  ...
  const a = Buffer.from((pw || "").toString());
  const b = Buffer.from(expected);
...
export async function setAdminCookie() {
  const store = await cookies();
  store.set(ADMIN_COOKIE, sign("ok"), {
    httpOnly: true,
    sameSite: "lax",
    secure: process.env.NODE_ENV === "production",
    path: "/",
